...
print(f"[INFO] Scanning for blockstates...")
files = glob.glob("pack/assets/minecraft/blockstates/*.json")
print(f"[INFO] Found {len(files)} blockstate files found.")
if files != []:
...
for file in files:
    ...
    if (block == "fire"): 
        print(f"[INFO] Skipping blocked block: {block}")
        continue
    ...
    with open(file, "r") as f:
        ...
        for k, v in data.items():
            if not("block/original" in v["model"] or "block/tripwire_attached_n" in v["model"]):
                print(f"  [DEBUG] Processing Variant: {k} | Model: {v['model']}")
                am = blocks_util.get_am_file(v["model"])
                if am == None: 
                    print(f"  [WARN] AM file not found for model: {v['model']} - Skipping.")
                    continue
                with open(am, "r") as f:
                    data_am = json.load(f)
                ...
                print(f"  [SUCCESS] Registered variant: {k}")
