...
jq -r '.[] | select(.generated == false) | [.path_hash, .path, .model_name] | @tsv' config.json | while IFS=$'\t' read hash path model_name; do
    ...
    if [[ "$texture_path" =~ (_cosmetic_normal_2|_cosmetic_self_2|_cosmetic_self\.png|_cosmetic\.png|_normal(_[0-9]+)?\.png|_self\.png|_[0-9]+\.png)$ ]]; then
        before="$texture_path"
        texture_path=$(echo "$texture_path" | sed -E 's/(_cosmetic_normal_2|_cosmetic_self_2|_cosmetic_self|_cosmetic|_normal(_[0-9]+)?|_self|_[0-9]+)\.png$/.png/I')
