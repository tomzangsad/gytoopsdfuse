...
class Crossbow_Util:
                ...
                "minecraft:attachable": {
                    "description": {
                        ...
                        "textures": {
                            ...
                            "crossbow_pulling_0": f"{textures[1]}",
                            "crossbow_pulling_1": f"{textures[2]}",
                            "crossbow_arrow": f"{textures[3]}",
                            "crossbow_firework": f"{textures[4]}",
                            "enchanted": "textures/misc/enchanted_item_glint"
                        },
                        "geometry": {
                            "default": f"{geometry[0]}",
                            "crossbow_pulling_0": f"{geometry[1]}",
                            "crossbow_pulling_1": f"{geometry[2]}",
                            "crossbow_arrow": f"{geometry[3]}",
                            "crossbow_firework": f"{geometry[4]}"
                        },
                        ...
                        "render_controllers": ["controller.render.crossbow_custom"]
                    }
                }
            }
            json.dump(data, f)
    ...
    def rendercontrollers():
        ...
        with open(file, "w") as f:
            data = {
                "format_version": "1.10",
                "render_controllers": {
                    "controller.render.crossbow_custom": {
                        "arrays": {
                            "textures": {
                            ...
                            "geometries": {
                                "array.crossbow_geo_frames": [
                                    ...
                                    "geometry.crossbow_firework"
                                ]
                            }
                        },
                        "geometry": "array.crossbow_geo_frames[math.floor(v.frame)]",
                        "materials": [ { "*": "variable.is_enchanted ? material.enchanted : material.default" } ],
                        "textures": [ "array.crossbow_texture_frames[math.floor(v.frame)]", "texture.enchanted" ]
                    }
                }
            }
            json.dump(data, f)
    ...
    def item_texture(gmdl, texture):
        # ฟังก์ชันนี้ถูกปิดใช้งาน ไม่ทำงานใดๆ
        print(f"[Warning] item_texture is disabled and does nothing.")

    def animation():
        with open("staging/target/rp/animations/crossbow_custom.animation.json", "w") as f:
            data = {
                "format_version": "1.8.0",
                "animations": {
                    "animation.player.crossbow_custom.first_person": {
                        "loop": True,
                        "bones": {
                            "rightitem": {
                    ...
                    "animation.player.crossbow_custom": {
                        "loop": True,
                        "bones": {
                            "rightitem": {
                                "position": [ 0.5, -2, 0 ]
                        ...
                        "states": {
                            "first_person": {
                                "animations": [
                                    ...
                                    {"first_person_crossbow_equipped": f"(query.get_equipped_item_name == 'crossbow' || query.is_item_name_any('slot.weapon.mainhand', {strlist})) && (variable.item_use_normalized > 0 && variable.item_use_normalized < 1.0)"},
                            ...
                            "third_person": {
                                "animations": [
                                    ...
                                    {"use_item_progress": f"( variable.use_item_interval_progress > 0.0 ) || ( variable.use_item_startup_progress > 0.0 ) && !variable.is_brandishing_spear && !variable.is_holding_spyglass && !variable.is_tooting_goat_horn && !query.is_item_name_any('slot.weapon.mainhand', 'minecraft:crossbow', {strlist})"},
                                    ...
                                    {"crossbow_controller": f"query.get_equipped_item_name == 'crossbow' || query.is_item_name_any('slot.weapon.mainhand', {strlist})"},
                                    {"third_person_crossbow_equipped": f"query.is_item_name_any('slot.weapon.mainhand', 0, 'minecraft:crossbow', {strlist}) && (q.is_using_item || q.is_charged)"},
